...
const SOME_SYSTEM_CREATED_GRADE =
	'tag:nextthought.com,2011-10:system-OID-0x32aa9a:5573657273:fSnNHAzMPbc';
const SOME_USER_UPDATED_GRADE =
	'tag:nextthought.com,2011-10:test.instructor@nextthought.com-OID-0x32aa9a:5573657273:fSnNHAzMPbc';
...
describe('NTIID Tests', () => {
	test('PropType validates NTIIDs', () => {
		...
		expect(() => PropType(props, propName, ComponentName)).toThrowError(
			message()
		);
		expect(() =>
			PropType.isRequired(props, propName, ComponentName)
	...
	test('encode throws if invalid id by default', () => {
		for (let bad of ['junk', 'fooboo']) {
			expect(() => decodeFromURI(encodeForURI(bad))).toThrowError(
				'Invalid NTIID'
			);
		}
	});
	...
	test('encode throws if invalid id with strict = true', () => {
		for (let bad of ['junk', 'fooboo']) {
			expect(() => decodeFromURI(encodeForURI(bad, true))).toThrowError(
				'Invalid NTIID'
			);
			expect(() => decodeFromURI(encodeForURI(bad, 1))).toThrowError(
				'Invalid NTIID'
			);
			expect(() => decodeFromURI(encodeForURI(bad, {}))).toThrowError(
				'Invalid NTIID'
	...
	test('encode does not throw if invalid id with strict = false', () => {
		for (let bad of ['junk', 'fooboo']) {
			expect(() =>
				decodeFromURI(encodeForURI(bad, false))
			).not.toThrowError('Invalid NTIID');
			expect(() => decodeFromURI(encodeForURI(bad, 0))).not.toThrowError(
				'Invalid NTIID'
			);
			expect(() =>
				decodeFromURI(encodeForURI(bad, null))
			).not.toThrowError('Invalid NTIID');

			//subtle (passing "undefined" to the arg triggers default value)
			expect(() => decodeFromURI(encodeForURI(bad, void 0))).toThrowError(
				'Invalid NTIID'
			);
			expect(() =>
				decodeFromURI(encodeForURI(bad, undefined))
			).toThrowError('Invalid NTIID');
		}
	});

	test('encode.sloppy() does not throw if invalid id', () => {
		expect(() => encodeForURI.sloppy('foobar')).not.toThrowError(
			'Invalid NTIID'
		);
	});
	...
	test('escapeId', () => {
		const expected =
			'tag\\3a nextthought\\2e com\\2c 2011-10\\3a system-OID-0x0c2b97\\3a 5573657273\\3a yApwFmaJ4a1';
